...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-717%20passed-2ea44f?logo=pytest" alt="Tests">
...
├── debate/          辩论编排器（D1-D4+M1-M4+R1+T1 九模块，9层链路 + TrustTracker 信任度）
├── data/            数据采集（缓存层 + 6 类数据 + Pydantic 模型 + MarketBrief 结构化简报）
...
├── backtest/        回测引擎（骨架 + TradePlan→TradeRecord 桥接适配器）
```

### 🧪 工程质量

- **717 测试全绿** — 单元测试 + 模块测试 + 辩论全流程（9层链路：分析师→策略师→辩论→评审→风控→交易员→PM裁决 + M2/M3/M4 反思+信任度+动态权重 + C1 简报分区） + 回测桥接 + 真实 LLM 集成测试
...
- [AI 工作流程](docs/01-guides/WORKFLOW.md) — 开发流程规范
