...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-436%20passed-2ea44f?logo=pytest" alt="Tests">
...
| **辩论引擎** | LangGraph StateGraph 完整五节点：数据收集 → 多大师分析(🧭 强制方向) → 交叉审阅+反驳 → **独立评审(D3)** → 加权投票聚合 + 方向分布 | ✅ MVP + D1+D2+D3 |
...
- **436 测试全绿** — 单元测试 + 模块测试 + 辩论全流程（交叉审阅+独立评审+方向约束） + 真实 LLM 集成测试
...
Phase 1 ──── MVP 链路 ██████████████████░░░  65% 🟡
  │  data/ ✅  debate/ ✅（D1交叉审阅 + D2方向约束 + D3独立评审）memory/ ✅  待完成：端到端接驳 · 前端
...
- 🧪 **436 测试** — 含真实 LLM 集成测试 + 辩论全流程，不是 mock 到死
