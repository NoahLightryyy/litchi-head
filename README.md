...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-491%20passed-2ea44f?logo=pytest" alt="Tests">
...
| **辩论引擎** | LangGraph StateGraph 完整链路：数据收集 → 多大师分析(🧭 D2方向) → D1交叉审阅 → D3独立评审 → D4加权聚合 + M1历史注入 | ✅ 五模块全部完成 |
...
├── debate/          辩论编排器（D1交叉审阅 + D2方向约束 + D3独立评审 + D4结构化扩展 + M1历史注入）
...
- **491 测试全绿** — 单元测试 + 模块测试 + 辩论全流程（交叉审阅+独立评审+方向约束+历史注入+结构化扩展） + 真实 LLM 集成测试
...
Phase 1 ──── MVP 链路 ██████████████████░░░  70% 🟡
  │  data/ ✅  debate/ ✅（D1+D2+D3+D4+M1 五模块）memory/ ✅  待完成：风控 · 前端
...
- 🧪 **491 测试** — 含真实 LLM 集成测试 + 辩论全流程，不是 mock 到死
