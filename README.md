...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-701%20passed-2ea44f?logo=pytest" alt="Tests">
...
- **701 测试全绿** — 单元测试 + 模块测试 + 辩论全流程（9层链路：分析师→策略师→辩论→评审→风控→交易员→PM裁决 + M2反思闭环 + M3信任度评分 + M4动态权重） + 真实 LLM 集成测试
...
- 🧪 **701 测试** — 含真实 LLM 集成测试 + 9 层完整链路，不是 mock 到死
