...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-517%20passed-2ea44f?logo=pytest" alt="Tests">
...
| **辩论引擎** | LangGraph StateGraph 完整链路：数据收集 → 分析师 → 策略师(D2方向) → D1交叉审阅 → D3独立评审 → D4加权聚合 + M1历史注入 → **R1三层风控 → PM裁决** | ✅ D1-D4+M1 完成 · R1 就绪 |
| **风控模块** | 三层风控辩论（Aggressive/Conservative/Neutral）+ PM 最终裁决 + 交易纪律体系 | ✅ R1 就绪（26 tests） |
...
- **517 测试全绿** — 单元测试 + 模块测试 + 辩论全流程（含三层风控+PM裁决） + 真实 LLM 集成测试
...
Phase 1 ──── MVP 链路 ██████████████████░░░  75% 🟡
  │  data/ ✅  debate/ ✅（D1-D4+M1+R1 六模块）memory/ ✅  risk/ ✅（R1 三层风控）待完成：前端
...
- 🧪 **517 测试** — 含真实 LLM 集成测试 + 辩论全流程（含风控层），不是 mock 到死
