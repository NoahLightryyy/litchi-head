...
## 2. 当前会话状态（2026-06-13 — TD-014 复杂度路由 + 分析师层收尾）

> **TD-014：新增 `src/utils/complexity_router.py` — DeepSeek chat/reasoner 自动切换。**
> **LLMConfig.reasoning_effort 已正确接入 `_build_llm()`，仅对 `deepseek-reasoner` 生效。**
> 辩论引擎 Phase 1 全部模块（D1/D2/D3/D4/M1 + 分析师层）已完成，491 tests。
...
🔧 修复中（2 条）：
  ...
  TD-004  测试基座（491 tests，backtest/risk 待补）
