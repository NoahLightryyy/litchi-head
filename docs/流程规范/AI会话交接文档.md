...
## 2. 当前会话状态（2026-06-14 — 架构批判与市场对照 + 优先级重排）

> **全架构 vs 市场三大路线逐层评判** — 识别反思闭环为首要战略缺口。
> **M2 反思闭环提升为 P0 第一优先** — 竞品最大结构性差距。
> 537 tests，全部模块就绪。新增 TD-017/018/019 三条债务。
...
## 5. 下一步优先级（2026-06-14 更新：架构审视 + 优先级重排）

> 基于 [[../架构设计/架构批判与市场对照]] 的市场对照分析，M2 反思闭环是当前与竞品的**最大结构性差距**——TradingAgents/ContestTrade/EvoTraders 都有反思机制，只有 litchi-head 停在裁决。

### 🥇 专业交易团队 — 下一步（重排后）

| 优先级 | 代号 | 说明 | 涉及模块 | 工作量 | 重排理由 |
|:------:|:----:|:-----|:--------|:------:|:-----|
| 🔴 **P0** | **M2 交易后反思** | 决策结果→实际收益→反思分析→更新记忆 | `memory/` + `debate/` | 中 | 竞品最大差距——缺少它就是"只说不练" |
| 🔴 **P0** | **回测引擎基础** | 简单策略回测框架 | `src/backtest/` | 中 | M2 反思需要回测数据验证决策质量 |
| 🟡 **P1** | **端到端链路验证** | 9 层全链路跑通 + 真实 LLM + 成本测量 | 多模块 | 中 | 验证层数膨胀的成本风险 |
| 🟡 **P1** | **M3 信任度评分** | 每位 Agent 输出 vs 实际结果准确率追踪 | `debate/` | 中 | M2 反思的评分数据来源 |
| ⬇️ **P2** | **C1 简报分区输出** | format_market_brief 按区块分区 | `data/collector.py` | 小 | 纯展示优化，不改变决策质量 |
...
> **最后更新**：2026-06-14（架构批判与市场对照 + 优先级重排 + TD-017/018/019 新增） | **如何更新**：每次会话结束时更新 §2 + §5 + 本行
