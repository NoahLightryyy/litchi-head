...
| 测试质量 | B+ | **222** 测试全绿（+20 LangGraph 原型测试） |
| 文档完整度 | A- | 14 份文档，结构清晰 |
...
✅ 已关闭（6 条）：
  ...
  TD-016  LangGraph 原型验证 ✅（20 测试，222 全量通过）

🔧 修复中（2 条）：
  TD-001  LLM 封装层（核心完成，模型路由待补）
  TD-004  测试基座（222 tests，debate 等模块待补）
...
| 002 | LangGraph 编排 | ✅ | 依赖已装，✅ 原型验证通过（TD-016 已关闭），适配器模式确认可行 |
...
```
记忆层（memory/）         ← 自动注入，永远在线，~2-3K
  ├── project-identity      技术栈/红线/定位（不改）
  ├── architecture-decisions ADR 摘要（ADR 变化时更新）
  └── current-state         当前任务/模块完成度（每次会话更新）
...
1. **不变信息不进文档** — 技术栈/红线/ADR 摘要写入 memory，跨会话自动注入
2. **现状写在最前** — 每次更新 `sessions/current-state.md`，读一条即可了解全貌
3. **按需不预读** — 不提前加载债务日志/设计文档，需要时才读

### 8.3 谁更新什么

| 文件 | 更新时机 | 更新人 |
|------|---------|--------|
| `project/project-identity.md` | 技术栈/红线变更时 | AI |
| `architecture/architecture-decisions.md` | ADR 新增/变更时 | AI |
| `sessions/current-state.md` | **每次会话结束时** | AI |
| 交接文档 §2 + §5 | **每次会话结束时** | AI |
...
> **最后更新**：2026-06-08 (TD-016 LangGraph 原型验证通过 ✅ — 最大架构风险解除) | **创建目的**：会话交接
