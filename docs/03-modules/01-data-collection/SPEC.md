...
25. 沪深生命周期必须来自交易所官方完整清单并保存原始响应哈希；北交所未验证前
    显式失败，不得复用沪深/CNINFO 路径。
...
|:-----|:----:|:------:|
| A股行情采集 | 已完成 | — |
| K 线数据采集 | KR-1A/B-1/B-2A/B/2C-1/2C-2A/2C-2B 完成；沪深生命周期、批次哈希与检查点生成可用，北交所/持久化待实现 | 58 |
| 新闻采集 | 已完成 | — |
| 数据缓存（TTL） | 已完成 | — |
| Pydantic 标准化转换 | 已完成 | — |
...
4. 🔥 KR-1B-2C-2C～3：北交所官方适配、长窗覆盖和持久化；
