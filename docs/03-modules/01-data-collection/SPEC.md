...
| `src/data/providers/cninfo_actions.py` | 深市/SSE 公司行动模板、重复日期表幂等、配股最终日程、差异化双现金口径；修订版唯一归链与 365 天同源历史回填 |
...
22. KR-3A 业务契约使用 `complete` 判别成功/失败：成功结果四层全部必填且深度
    冻结；失败结果只携带四层诊断和稳定错误码，不得夹带半成品行情。收盘晋升以
    内容哈希幂等，同一证据重试返回原记录，不同证据拒绝覆盖。
23. KR-3B-1 成功组装必须先验证三类证据信封均为完整、能力槽位与请求证券一致；
    `daily_snapshot_id` 必须等于复权序列的 `raw_snapshot_id`；`complete=false` 的
    残留条目不得读取，只允许 `FINAL` 分钟进入完成层，动态日条必须从双源 RAW
    实时报价构造并保持 `PROVISIONAL`。
24. 停复牌公告候选必须下载官方附件并解析明确生效日，保存附件 URL 与原始文件
    SHA-256；标题推断、正文无日期或附件失败均不得生成成功状态。
25. `OfficialSuspensionEvent` 只是状态转换证据；在连续事件账本、生命周期边界和
    查询覆盖证明完成前，不得直接生成生产 `OfficialSecurityStatusWindow`。
26. 状态检查点必须携带已公告但尚未生效的转换；账本先回放窗口前事件，再计算窗口
    首日状态。查询批次按自然日连续，断档、同日相反转换和锚点不足均失败关闭。
27. 重复停牌/复牌公告不重复切换状态，但来源 URL 与 SHA-256 全部进入输出血缘。
28. 三市场生命周期必须来自各交易所独立官方入口并保存完整响应集哈希；北交所
    额外校验新旧代码映射、产品类型、分页与分类计数。0600/0700 表示全天转换，
    9001 盘中临停不得剔除整日日线。缺结构化上市日的历史转板股必须显式失败。
29. 检查点生成只能消费覆盖结束日不晚于目标日的完整批次；检查点哈希包含生命周期、
