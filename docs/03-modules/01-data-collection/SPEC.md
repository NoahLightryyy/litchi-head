...
|:-----|:----:|:------:|
| A股行情采集 | 已完成 | — |
| K 线数据采集 | KR-1A RAW 双源严格对账完成；KR-1B 完备性/持久化与下游串联待实现 | 25 |
| 新闻采集 | 已完成 | — |
| 数据缓存（TTL） | 已完成 | — |
| Pydantic 标准化转换 | 已完成 | — |
...
3. 🔥 KR-1B：经批准的交易日历/个股停牌状态、预期日期集、长窗覆盖和持久化；
4. KR-2：RAW、公司行动/因子和派生复权序列分层并版本化；
5. KR-3：汇总 `FINAL_DAILY / FINAL_MINUTE / LIVE_QUOTE / PROVISIONAL` 分层信封；
6. 保持正式指标与盘中估算指标字段隔离，覆盖晋升、冲突、未来因子和失败关闭；
7. K 线全链路完成后继续迁移行业证据。
