...
|:-----|:----:|:------:|
| A股行情采集 | 已完成 | — |
| K 线数据采集 | KR-1A/B-1/B-2A/B/2C-1/2C-2A/2C-2B/2C-2C 完成；三市场生命周期、批次哈希与检查点生成可用，长窗持久化待实现 | 76 |
| 新闻采集 | 已完成 | — |
| 数据缓存（TTL） | 已完成 | — |
| Pydantic 标准化转换 | 已完成 | — |
...
5. 🔥 KR-1B-3：长窗覆盖、RAW/诊断/状态版本持久化和回放；
6. KR-2：RAW、公司行动/因子和派生复权序列分层并版本化；
7. KR-3：汇总 `FINAL_DAILY / FINAL_MINUTE / LIVE_QUOTE / PROVISIONAL` 分层信封；
