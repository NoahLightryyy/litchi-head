...
1. 沪深使用新浪+腾讯两个独立上游核验 RAW 完成日线；北交所缺第二上游时失败关闭；
2. RAW OHLC 按 `price_tick` 严格一致，成交量按来源精度对账；实时/分钟容差不得复用；
3. RAW、公司行动/因子和派生复权序列分层并版本化，由项目统一生成点时前复权；
4. 汇总 `FINAL_DAILY / FINAL_MINUTE / LIVE_QUOTE / PROVISIONAL` 分层信封；
5. 保持正式指标与盘中估算指标字段隔离；
6. 覆盖开盘可用、收盘晋升、来源/RAW/复权冲突、未来因子和失败关闭测试。
5. 完成后继续迁移行业证据。
