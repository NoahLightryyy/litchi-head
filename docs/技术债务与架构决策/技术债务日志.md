...
```
S0 🔴 致命 │█ 1   ──→ 立即关注
S1 🟠 严重 │█ 1    ──→ 本周修复（TD-012 ✅ 已关闭）
S2 🟡 中等 │████ 4   ──→ 本周/本 Sprint  ← +2
S3 🟢 轻微 │████ 4   ──→ 有计划即可
S4 ⚪ 建议 │      ──→ 0
...
```
开放债务本金: ~9.9 人时  (← 关闭 TD-012 减约 1d)
最高利息:   TD-013 无 streaming 接口 / TD-015 缓存不支持多配置 — 辩论引擎上线前应修复
紧急指数:   0.9/10 —— 较前次 1.2 继续下降，因 TD-012 已关闭；剩余 2 项（TD-013/015）待修复
总开放债务: 9 条（5 条已关闭）
```
