...
| **状态** | `🔧 修复中` |
| **本金估算** | ∼2h |
| **实际工时** | ∼30min（conftest.py + test_sanity.py + vcr_config.py 核心架子已完成） |
| **日利息** | 🟡 提升至 24 tests collected，但真实 Agent 业务测试尚未编写 |
...
2. ✅ `tests/conftest.py` — 共享 fixtures（`mock_llm`, `sample_context`, `sample_result`, `sample_message`, `mock_llm_sequence`, `mock_llm_error`）
3. ✅ `tests/conftest.py` — pytest 配置（marker 注册 + 异步测试自动适配）
4. ✅ `tests/vcr_config.py` — VCR 录制/回放配置
5. ✅ `tests/test_sanity.py` — 24 个冒烟测试覆盖所有工具函数和 fixtures
6. ⬜ 各模块的正式业务测试（test_agents, test_debate 等）

**利息分析**：
- CI 跑 pytest → 24 tests collected ✅
- 已有测试基线，可安全重构已有代码
- 但核心 Agent 业务逻辑仍未测试
...
| TD-004 | 缺少测试基座 | 🧪 testing | 🟡 S2 | tests | ∼2h | 🟡 基础架子已搭好，正式业务测试待补 | 🔧 修复中 |
...
本次会话（修复 CI 全部检查项 + 关闭两条债务）：

| ID | 操作 | 说明 |
|----|------|------|
| TD-001 | 利息更新: 🟡 → 🟢 | CI 全绿通过后 LLM 封装层风险进一步降低 |
| TD-004 | 无变更 | 测试基座已可用，正式业务测试待补 |
...
最高利息:   TD-004 业务测试待补
紧急指数:   2.0/10 —— CI 全绿通过 ✅，两条债务关闭，最坏的情况已经过去
```
