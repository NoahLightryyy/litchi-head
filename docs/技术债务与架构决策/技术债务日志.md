...
| **状态** | `📋 已确认` |
| **本金估算** | ∼2h |
| **日利息** | 🔴 迁至 GitHub 后 CI 真空期，无测试则流水线形同虚设 |
...
- 迁至 GitHub 后尚无 CI 配置，测试缺失的利息从 🟡 升至 🔴
- 无法安全重构已有代码
...
| **状态** | `🔧 修复中` |
| **本金估算** | ∼30min |
| **实际工时** | ∼5min（创建 `.github/workflows/ci.yml`） |
| **日利息** | 🟡 CI 文件已创建，但尚未在 GitHub 上验证运行 |

**描述**：
项目已从 Gitee 迁移到 GitHub，但 CI 流水线仍是 Gitee Go 格式（`.gitee/workflows/ci.yaml`），GitHub 上无法运行。

**具体问题**：
1. ❌ ~~`.gitee/workflows/ci.yaml` — Gitee Go 格式~~（无该文件，Gitee CI 从未配置过）
2. ✅ `.github/workflows/ci.yml` — 已创建 GitHub Actions 配置文件
3. ⬜ 尚未在 GitHub 上验证运行

**利息分析**：
- 每次 push 到 GitHub 没有自动代码检查
- Ruff/Pyright/Pytest 全靠手动本地运行
- 多人协作时质量无法保证

**修复方向**：
- 创建 `.github/workflows/ci.yml`，等效迁移 Gitee Go 流水线内容
- 保留 `.gitee/` 目录（作为 Gitee 备份远程的 CI 配置）
...
| TD-004 | 缺少测试基座 | 🧪 testing | 🟡 S2 | tests | ∼2h | 🔴 迁至 GitHub 后利息升高 | 📋 已确认 |
...
| TD-009 | CI 需迁移 GitHub Actions | ⚙️ infra | 🟡 S2 | infra | ∼30min | 🟡 CI 文件已创建，待验证 | 🔧 修复中 |
...
本次会话（创建 GitHub CI + 修复 Pyright 路径）：
...
```
总本金:   ~9.4 人时  (≈ 1.2 人天)
最高利息: TD-009 CI 尚未验证 + TD-004 测试缺失
紧急指数: 3.0/10 —— CI 文件已创建但未验证，测试基座仍是最大短板
```
...
> | 2026-06-06 | 更新 TD-004 | 利息评估从 🟡 升至 🔴（迁至 GitHub 后 CI 真空期） |
> | 2026-06-06 | 新增 TD-009~011 | CI 迁移 / README 模板 / Pyright 路径（全面审视发现） |
> | 2026-06-06 | 仪表盘更新 | 更新所有统计指标反映新增 3 条债务 |
> | 2026-06-07 | 更新 TD-009 | 状态变更：📋 已确认 → 🔧 修复中（创建 `.github/workflows/ci.yml`） |
> | 2026-06-07 | 更新 TD-011 | 状态变更：📋 已确认 → ✅ 已关闭（移除 hardcoded extraPaths） |
