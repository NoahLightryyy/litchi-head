...
| **发现人** | AI 全面项目审视 |
| **状态** | `📋 已确认` |
| **本金估算** | ∼0.5d（编写最小原型验证 + 修复接口不匹配） |
| **日利息** | 🔴 每日不验证，Phase 1 启动后可能被迫大规模重构 |
...
**具体问题**：
1. ❌ 无任何 `StateGraph` 实例 — 编排层完全不存在
2. ❌ 无 `add_node()` / `add_edge()` — 接口设计与 LangGraph API 的匹配度未验证
3. ❌ `BaseAgent.run()` 接口能否直接作为 LangGraph 节点未知
4. ❌ `AgentContext` 作为 StateGraph 共享状态的兼容性未测试

**利息分析**：
- 当前利息低（Phase 0 不需要编排器）
- **Phase 1 启动辩论引擎时利息骤升为致命**：
  - LangGraph API 若与接口设计不匹配 → 修改所有 Agent 接口 → 重跑全部测试
  - 严重情况下可能推倒 ADR-002 选替代框架
- 修复成本极低（半天）但完全不做风险极高

**修复方向**：
编写 LangGraph 最小原型，验证：
1. `StateGraph` 基础搭建是否与项目结构兼容
2. `BaseAgent.run(AgentContext) -> AgentResult` 能否直接作为节点
3. `AgentContext` 能否序列化为 StateGraph 的 State
4. 节点间消息传递是否走通
...
| TD-016 | LangGraph 零使用未验证 | 🏛️ design | 🟠 S1 | core | ∼0.5d | 🔴 Phase 1 启动后可能被迫大规模重构 | 📋 已确认 |
...
| ID | 操作 | 说明 |
|----|------|------|
| TD-016 | 🆕 新增 | LangGraph 零使用未验证 — Phase 0→1 最大单一风险 |
...
```
S0 🔴 致命 │█ 1   ──→ 立即关注
S1 🟠 严重 │█ 1    ──→ 本周修复（TD-016 LangGraph 验证）
S2 🟡 中等 │█████ 5  ──→ 本周/本 Sprint
S3 🟢 轻微 │████ 4   ──→ 有计划即可
S4 ⚪ 建议 │      ──→ 0
✅ 已关闭 │█████ 5   ──→ TD-002 + TD-009 + TD-011 + TD-012 + TD-014
```
...
```
开放债务本金: ~10.4 人时  (← 新增 TD-016 加 ~0.5d)
最高利息:   TD-016 LangGraph 验证 —— Phase 1 启动前未验证可能导致大规模重构
紧急指数:   1.2/10 —— 较前次 0.9 回升，因新增 TD-016（LangGraph 未验证）
总开放债务: 10 条（5 条已关闭 ✅）
```
