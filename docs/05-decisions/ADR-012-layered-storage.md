...
3. **Parquet 行情目录**
   ...
   - 按市场、品种、周期、日期分区；
   - K 线按 RAW、公司行动/因子和派生复权分层，记录来源、字段精度、`as_of`、
     `factor_version` 与校验状态；派生层可重建，RAW 和因子版本不可静默覆盖；
   - 不把高容量时序数据强塞进普通业务事务表。
...
- 采用 SQL 事实源 + Redis 协调层 + Parquet 行情目录；
- 采用调研报告第 3 节的数据生命周期默认基线。

该确认锁定架构边界和产品默认值，不代表已经部署数据库，也不构成未经实测的
容量或并发承诺。
...
7. K 线 RAW/因子/派生分区完成版本重放，证明历史回测不能读取未来因子。
