...
和费用，不证明完整市场数据输入下的分析质量。该行为已登记 TD-069。产品侧已于
2026-07-28 确认核心证据失败关闭，并于 2026-07-30 确认盘中 K 线双时间尺度及
RAW/公司行动/派生复权分层：完整历史日 K 与实时/分时/`PROVISIONAL` 今日状态
分层进入 AI，历史回测只能读取当时可得因子。具体实施见
[ADR-013](../05-decisions/ADR-013-multi-source-evidence.md) 和
