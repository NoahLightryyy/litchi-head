...
```
src/
├── debate/     辩论引擎     —— 4 组辩论 + 大师投票 + 交叉质疑
├── data/       数据采集     —— akshare 行情 + 新闻
├── backtest/   回测引擎     —— 策略回测
...
| 步骤 | 事项 | 前置 |
|:----:|:-----|:----:|
| 1️⃣ | **辩论编排器 LangGraph StateGraph** — 第一步（编排器基础框架） | TD-014 ✅ + TD-016 ✅ |
| 2️⃣ | **辩论分组** — 4 组大师分组辩论逻辑 | 编排器就绪 |
| 3️⃣ | **交叉质疑** — Agent 之间互相质疑机制 | 辩论基础就绪 |
...
| 优先级 | 事项 | 依赖 | 预估 |
|:------:|:-----|:----|:----:|
| 🥇 | **端到端链路验证** — 用户问题 → MasterAgent → 多 Agent 辩论 → 决策卡输出 | 辩论引擎就绪 | ~3d |
| 🥇 | **前端 MVP（3 页面）** — 首页/分析页/我的页面（Streamlit） | 端到端链路就绪 | ~3d |
| 🥇 | **数据采集接入** — akshare 实时行情 + 新闻 | debate/data 就绪 | ~2d |
| 🥈 | **用户行为镜子 Agent** — 记录期（1-9 次决策）MVP | 辩论引擎就绪 | ~2d |
| 🥈 | **回测模块基础** — 简单策略回测 | debate/data 就绪 | ~2d |
