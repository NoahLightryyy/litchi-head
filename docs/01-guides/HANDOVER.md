...
| 🔥 P0 | **ADR-012 数据生命周期与持久化底座** — 恢复、CNINFO 三态、新闻双源与统一聚合 API 完成，待接正式失败关闭 |
...
1. 东方财富与新浪新闻适配器已通过统一 `SourceResult[NewsItem]` 接入；
2. `/api/v1/evidence/news/aggregate` 并发采集、跨源去重并返回完整逐源诊断；
3. 新浪历史不足以覆盖时间窗时显式返回 `STALE`，不会伪装成功空结果；
4. 下个原子功能把新闻 `EvidenceEnvelope` 接入正式辩论数据节点；
5. 证据不完整时必须在 LLM 前失败关闭，并验证调用数为零。
...
> **最后更新**：2026-07-29 | 东方财富 + 新浪新闻双源聚合完成；下一步接正式辩论失败关闭
