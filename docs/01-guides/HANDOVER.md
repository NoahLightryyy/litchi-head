...
| **最新提交** | 待提交（本次为 docs 重组 + ADR/债务拆分，见工作日志 2026-06-15） |

---

## 2. 当前会话状态（2026-06-15 — docs 目录重组完成）

> **本次完成了 `docs/` 目录的大规模重组**。按「受众 × 生命周期」重新组织。旧目录加 `.legacy` 后缀保留内容。
...
docs/
├── README.md                 ← 全文档路由表
├── 00-overview/              ← 🏠 总览（4 文件，不膨胀）
├── 01-guides/                ← 📐 流程规范 + AI 路由
│   └── debt/                 ← 债务按类型拆分（7 文件）
├── 02-requirements/          ← 📋 产品需求
├── 03-modules/               ← 🔧 ★ 核心：9 个模块，各一个文件夹
│   ├── 01-data-collection/   ← README + SPEC + ADR
│   ├── 02-debate-engine/     ← README + SPEC + ADR
│   └── ...（同格式）
├── 04-changelog/logs/        ← 📋 AI 工作日志（39 文件，按日平铺）
├── 05-decisions/             ← 🏛️ 跨模块 ADR（6 文件）
└── 99-archive/               ← 🗄️ 归档（11 文件）
```

**核心思想**：一个功能 = 一个文件夹。开发辩论功能只看 `03-modules/02-debate-engine/`。
...
| 日志 4 级嵌套（39 文件遍历累） | ✅ 按日平铺 |
| 中英混用目录名 | ✅ 全英文路径 |
| 旧/新内容混在一起 | ✅ 旧内容移入 `.legacy` 目录 |

### 尚未完成的

1. **`01-guides/ROUTING.md`** — 上下文加载策略文件，预留了位置但还没写
2. **SPEC.md 内容精简** — 9 个模块的 SPEC 目前是原调研分析 README.md 的直接复制，未做精简
3. **`.legacy` 目录删除** — 确认新结构没问题后可清理

### 当前 Git 状态

```
已提交: 31fec4c — docs: P0 端到端链路验证 + Batch Loop 规则 + 上下文耗尽自动交接
工作区: docs/ 目录重组 + ADR/债务拆分（未提交）
```
...
> **本次完成**：docs/ 目录按「受众 × 生命周期」重组，ADR 拆分，债务按类型拆分。

### 🥇 下一步

| 优先级 | 说明 | 涉及范围 | 工作量 |
|:------:|:-----|:--------:|:-----:|
| 🟡 **P1** | **01-guides/ROUTING.md** — 上下文加载策略，定义按任务的文件加载规则 | `01-guides/` | 小 |
...
2. 写 `ROUTING.md`（新结构的最后一块拼图）
3. 清理 `.legacy` 目录
4. 继续功能开发
```

> **最后更新**：2026-06-15（docs 目录重组完成） | **如何更新**：每次会话结束时更新 §2 + §5 + 本行
