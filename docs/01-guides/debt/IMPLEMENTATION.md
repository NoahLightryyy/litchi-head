...
**2026-07-28 实测补充**：
一场真实五分析师/五大师/五互评辩论执行 16 次 LLM 调用，输入 52,982 Token、
输出 16,708 Token，耗时约 175.8 秒，按当日兼容模型价格记录 ¥0.082006。
当时即使实时行情、新闻和行业数据采集失败，完整 LLM 链仍会执行。2026-07-29
新闻和实时行情已接入零 LLM 失败关闭。2026-07-30 已确认 K 线双时间尺度、RAW
双源、统一点时复权和冲突口径：完整历史日 K 与实时/分时/`PROVISIONAL` 今日状态
分层进入盘中 AI；完成日线 RAW 严格对账，复权血缘独立保存。K 线按
[KR-1～KR-6](../../02-requirements/KLINE_EVIDENCE_IMPLEMENTATION_PLAN.md) 实施，
行业数据剩余工作见部门债务 TD-069。
...
| **本金估算** | ∼1h |
| **修复日期** | 2026-06-18 |
| **修复说明** | 备用模式每次先尝试主源，成功则自动切回 — `fallback.py:_call()` 重构 |
...
**2026-07-28 进展**：
已修复结构化 LLM 调用不记录真实 Token 的问题，并按 DeepSeek 当日价格更新
缓存命中/未命中计价。债务不关闭：价格仍是代码常量，供应商调价仍需重新部署。
