...
**修复方案**：
1. ✅ `src/utils/credentials.py` — 凭据管理器模块，使用 keyring 读写系统凭据管理器
2. ✅ `src/utils/config.py` — Settings 初始化后从凭据管理器覆盖 API Keys（优先级：凭据管理器 → 环境变量 → .env）
3. ✅ `scripts/store-api-keys.py` — 交互式密钥存储工具
4. ✅ 密钥已迁移至 Windows Credential Manager
5. ✅ `.env` 清空明文密钥，仅保留占位符
6. ⬜ **用户需轮换密钥**：登录 DeepSeek/Anthropic 平台生成新密钥，运行 `python scripts/store-api-keys.py` 更新

**已知旧密钥**：已在 `.env` 中存在超过 2 周，建议轮换。
**凭据管理器服务名**: `litchi-head`
