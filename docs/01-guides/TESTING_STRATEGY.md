...
├── test_backend/
│   ├── __init__.py
│   ├── conftest.py                 # FastAPI TestClient + MockCollector + mock 工厂
│   ├── test_indicators.py          # ✅ 已有（422 行，100% 覆盖）
│   ├── test_market.py              # ✅ 52 测试（6 端点 + 5 辅助函数）
│   ├── test_stocks.py              # ✅ 15 测试（6 端点）
│   ├── test_debate.py              # ✅ 9 测试（3 端点 + session 生命周期）
│   └── test_trust.py               # ✅ 11 测试（2 端点 + 映射逻辑）
│
├── test_backtest/
│   ├── __init__.py
...
| `test_backend/conftest.py` | ✅ **已完成** — 77 测试覆盖 17 端点 | ✅ |
...
| `contract/` 目录 | ✅ **已有** — 4 项 data→debate 契约测试 | ✅ |
...
| `src/risk/` | ~65% | ≥80% | 🔴 | |
| `src/backtest/` | ~75% | ≥80% | 🟡 | |
| `backend/` | ~75% | ≥80% | 🟡 | 路由 HTTP 测试已覆盖（77 测试），coverage 测量需加 --cov=backend |
...
> **最后更新**：2026-06-18（TD-036 完成后同步 — backend 路由测试状态 ✅）
