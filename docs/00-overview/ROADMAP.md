...
```
总 Python 测试数 │ 1692 collected；1669 passed / 4 skipped / 19 deselected
...
| 🔥 P0 | **TD-069 / KR-3～4 双时间尺度与 AI 串联** — KR-3A 四层冻结契约/幂等晋升 ✅；下一步 KR-3B 运行时组装，再做辩论/Agent/风控/交易门禁 | KR-2 | ~1.25d |
...
- ✅ **DP-006 完成** — 镜子反思，`MirrorReport` 模型 + `mirror.py` 核心逻辑 + orchestrator 集成 + 19 测试
...
| 优先级 | 事项 | 牵头 | 预估 |
|:------:|:-----|:------|:----:|
| 🔥 P0 | **FD-001 基本面数据接入** — ✅ 全部完成（模型 ✅ 多源财务数据 ✅ 辩论注入 ✅ 分析师增强 ✅ API 端点 ✅ 前端 Tab） | 全部门 | 无 | ~0 剩余 ⏱️ |
| 🔥 P0 | **PD-001/002/003 动态指标体系** — ✅ IndicatorRegistry + 产业链位置 + 选择器（31 行业 × 5-8 指标，455 条映射） | 数据管道部 ✅ | 无 | ~5h，34 tests ✅ |
| 🔥 P0 | **PD-004 前端行业感知** — ✅ FinancialPanel 按注册表动态过滤指标，行业标签 + 产业链位置显示 | 前端部 ✅ | 无 | ~1h，325 tests ✅ |
...
| ✅ P1 | **TD-041 数据新鲜度标注** — ✅ 已完成：fetched_at + 前端标签 + 缓存标记 | 数据管道部+前端部 | ~2h ✅ |
| ✅ P2 | **DP-006 镜子反思** — ✅ 已完成：历史对比，mirror.py + 19 tests | 辩论引擎部 | ~3h ✅ |
...
6/23                                         8/31
  │                                            │
  ├── [DLC] 美股 ────────── 移至 DLC ──→ ⏸️    │
  ├── P0 FD-001 基本面 ──── 1 天(模型✅) + 1 天(多源✅+注入+分析师✅) + 0.5 天(前端✅) ──→ ✅ 全部完成  │
├── P0 PD-001/002/003 动态指标体系 ── 1 天(API验证✅+注册表✅+选择器✅+34测试✅) ──→ ✅ 基建完成  │
├── P0 PD-004 前端行业感知 ── 1h(FinancialPanel 指标过滤✅+行业标签✅+API端点✅) ──→ ✅ 完成  │
  ├── P0 置信度量化 ─────── 2 天 ──→ ✅         │
  ├── P0 复盘看板(极简) ── 2 天 ──→ ✅         │
...
| FD | 事项 | 牵头 | 依赖 | 预估 |
|:--:|:-----|:----|:----|:----:|
| **FD-001** 🥇 | **基本面数据接入** — ✅ 全部完成（模型+Provider+多源财务数据+辩论注入+分析师增强+API 端点+前端 Tab） | 全部门 | 无 | ✅ 已完成 ~2 天 |
| **FD-002** 🥇 | **估值比率模型** — PE(市盈率)/PB(市净率)/PS(市销率)，纯计算模型，不依赖新 Provider | 数据管道部 | FD-001 数据层就绪 | ✅ 已完成 |
| **FD-003** 🥇 | **产业链修复** — 用真实行业分类替换当前伪产业链数据（涨幅排序造假） | 后端 API 部 + 前端部 | 现有 akshare 行业分类 | ~半天 |
| **FD-004** 🥈 | **供应链图谱调研** — 评估年报 PDF 解析前5大客户/供应商可行性 | 数据管道部 | FD-001 完成 | ~2h |
| **FD-005** 🥉 | **供应链图谱**（条件性）— NLP Pipeline 解析年报 → 可视化图谱 | 数据管道部 + 前端部 | FD-004 调研结论 | ~5-7 天 |

**关键里程碑**：
```
FD-001 ✅ 2.5天完成（全链路：模型+Provider+多源注入+分析师增强+API+前端Tab）
  ├─ ✅ 数据模型 FinancialMetrics + Provider + Collector → 数据层就绪（7/23）
  ├─ ✅ 多源财务数据 ADataSource+ZzshareSource get_financials()（FD-001h）
  ├─ ✅ 基本面占位符填充 → 辩论质量提升（FD-001e）
  ├─ ✅ API 端点 /financials + /valuation + 前端 Tab（FD-001i）
  └─ ✅ FD-002 估值比率模型（PE/PB/PS）
       ↓
FD-003 ✅ 半天完成（修复伪产业链 = 消除数据诚信债务）
       ↓
FD-004 ❓ 调研决策点（供应链图谱做不做？）
```
...
| 架构决策 | `docs/05-decisions/README.md` | ADR 索引（含已批准方向的 ADR-012 分层存储） |
...
| 2026-07-23 | **FD-001 Step 1 财务指标数据模型** — FinancialMetrics 17 指标 + DataSource 协议扩展 + AKShareSource 实现 + DataCollector 封装，28 测试 |
| 2026-07-23 (2) | **FD-001 Step 2 基本面数据注入辩论引擎** — format_market_brief 填充真实财务数据 + collect_data_node 采集 + 分析师自动受益，+6 测试，261 辩论全模块通过 |
| 2026-07-23 (3) | **FD-002 估值比率模型** — ValuationMetrics(PE/PB/PS) + StockQuote.market_cap + DataCollector.get_valuation() + 17 测试，1029 全量通过 |
...
| **2026-07-28** | **ADR-012 方向获批 + 恢复门禁 + 多源证据契约** — 用户确认三层存储、生命周期和证据不完整时失败关闭；SQLite/PostgreSQL 恢复、真实 LLM 容量、LangGraph 续跑通过；统一来源身份、六态结果和独立上游完整性评估基础完成 |
| **2026-07-29** | **东方财富 + 新浪新闻双源聚合** — 两源并发、跨源去重、统一 `EvidenceEnvelope` 与 `/api/v1/evidence/news/aggregate` 完成；新浪历史不足时显式 `STALE`，不伪造成功空结果 |
| **2026-07-29 (2)** | **新闻证据正式失败关闭** — 新浪 SQLite WAL 滚动采集 3 天元数据；东方财富实时并发；缺源返回 503，首个 LLM 前终止并验证零调用 |
| **2026-07-29 (3)** | **实时行情双源失败关闭** — 东方财富 + 新浪直连并发；成交量统一为股；10 秒新鲜度、3 秒配对、0.01 元容差；统一聚合 API 与辩论零 LLM 门禁完成 |
| **2026-07-29 (4)** | **L1 分时战况一期** — 东方财富 + 腾讯分钟序列双源对账；上游“手”统一为“股”；当前分钟 `PROVISIONAL`；VWAP/开盘区间统一业务信封；L1 明确禁止身份归因 |
| **2026-07-30** | **盘中 AI 双时间尺度方向确认** — AI 不等待日 K 收盘；历史完整日 K、实时行情、已结束分钟和 `PROVISIONAL` 今日动态状态分层输入；动态状态不得冒充已确认日线形态 |
| **2026-08-03** | **TD-072 分时同期量能底座** — 腾讯五日历史只进影子层；双源242分钟完整日进正式层；Parquet+SQLite 哈希失败关闭；20日中位数未暖机前 Relative Volume 继续不可用 |
| **2026-08-03 (2)** | **KR-2B-2B2A/2B2B 公司行动官方模板与修订归链** — SSE 普通/差异化、配股最终日程、双现金口径、不可变修订账本、365 天同源历史回填及歧义/倒序失败关闭完成；下一原子 2C |
| **2026-08-03 (3)** | **KR-2B-2C 已核验因子转换核心** — 相邻累计除数、官方事件与登记日 RAW 公式复算；用户批准 12 位 `ROUND_HALF_EVEN` 门限；`000001` 三事件实网通过。沪市版式缺口登记 TD-075，KR-2 尚未完成 |
| **2026-08-04** | **TD-075 关闭 / KR-2 完成** — SSE 重复日期表幂等、修订版与完整标题空白归一、差异化扣减回购股本公式；`600000/688008/688503` 真实全链和三上游故障注入通过；下一阶段 KR-3 |
| **2026-08-04 (2)** | **KR-3A 四层业务契约与收盘晋升核心** — 成功/失败判别联合、动态/完成层隔离、深度冻结 RAW 实时事实、确定性 promotion ID、幂等重试与冲突拒绝；下一原子 KR-3B 运行时组装 |
