...
| 优先级 | 事项 | 牵头 | 预估 |
|:------:|:-----|:------|:----:|
| 🔥 P0 | **FD-001 基本面数据接入** — ✅ 模型+Provider ✅ 多源财务数据 ✅ 辩论注入 ✅ 分析师增强 ⬜ 前端展示 | 数据管道部→辩论引擎部→后端 API 部→前端部 | ~0.5 天剩余（前端） |
...
6/23                                         8/31
  │                                            │
  ├── P0 美股数据源 ─────── 1 天 ──→ ✅         │
  ├── P0 FD-001 基本面 ──── 1 天(模型✅) + 1 天(多源✅+注入+分析师✅) + 0.5 天(前端⬜) ──→ 🟡         │
  ├── P0 置信度量化 ─────── 2 天 ──→ ✅         │
  ├── P0 复盘看板(极简) ── 2 天 ──→ ✅         │
...
| FD | 事项 | 牵头 | 依赖 | 预估 |
|:--:|:-----|:----|:----|:----:|
| **FD-001** 🥇 | **基本面数据接入** — ✅ 模型+Provider ✅ 多源财务数据(AData+Zzshare) ✅ 辩论注入 ✅ 分析师增强 ⬜ 前端展示 | 数据管道部 → 辩论引擎部 → 后端 API 部 → 前端部 | 无（akshare 已有接口） | ~2 天 |
...
**关键里程碑**：
```
FD-001 ✅ 2天完成（全链路：模型+Provider+多源注入+分析师增强）
