...
department: 基础设施部
codebase: src/utils/
last_updated: 2026-07-30 (K 线来源配置与可观测职责确认)
---
...
| TD-070 | FastAPI TestClient 与 py_mini_racer 使用已弃用依赖接口 | 🟢 | 1h |
...
|:------:|:-----|:----:|
| 1 🟡 | TD-005 双配置源协调 | 无 |
| 2 🟢 | TD-070 核对测试依赖兼容矩阵并消除上游弃用警告 | 无 |
| 3 🟢 | TD-007/TD-008/TD-055 小修 | 无 |
| 4 🔥 | KR-6 按市场/上游/能力暴露 K 线健康、冲突和最后成功时间；隔离实时/分钟/完成日线配置 | 3B 已接入审计目录；负责 `LITCHI_KLINE_AUDIT_ROOT` 绝对路径、SQLite+Parquet 整体备份/恢复、空间与完整性巡检。不得另建第二套状态/行情目录 |
...
基础设施部负责冻结模型、配置、Prompt 和提交版本，记录费用、延迟、失败与恢复结果，
并保证决策账本备份恢复和一键启动。模型或配置变更必须开启新实验版本。
