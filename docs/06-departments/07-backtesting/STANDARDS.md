...
- 回测日 `t` 只能读取在 `t` 当时已经发布的公司行动和因子版本；
- 禁止用今天锚定的前复权序列回放历史决策；
- 交易成交、滑点和费用使用 RAW 价格；技术指标使用 `ADJUSTED_QFQ_ASOF`；
- 每份报告记录 RAW 数据版本、`factor_version`、`reference_date`、`as_of` 和来源覆盖；
- 因子修订后的重跑必须生成新报告版本，不静默覆盖旧结果；
- 缺独立 RAW 来源或因子血缘时，实盘质量回测失败关闭。
