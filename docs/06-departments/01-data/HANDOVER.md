...
department: 数据管道部
codebase: src/data/
last_updated: 2026-07-30 (KR-1B-3A 不可变审计持久化与点时回放完成)
---
...
| 16 🔥 | K 线 KR-1B-3B 长窗覆盖 | 真实新浪/腾讯逐源覆盖证明与运行时接入；不可证明即失败关闭 |
| 17 🔥 | K 线 KR-2 统一复权 | RAW/公司行动/派生序列分层；点时因子 |
...
6. KR-1B-3A 已完成，统一复用 `src/data/kline_store.py`；不得另建 K 线清单、
   Parquet 目录、逻辑快照哈希或 `as_of` 回放；
7. 下一步只做 KR-1B-3B：证明新浪/腾讯各自长窗覆盖并接入运行时。新浪现有端点
   无法证明任意历史分页时失败关闭；换源/加源属于方向决策，必须先停下确认；
8. KR-1B 真实烟测和故障注入通过后才进入 KR-2；不得同时提前改前端；
9. KR-2 通过后按 KR-3 输出 `FINAL_DAILY / FINAL_MINUTE / LIVE_QUOTE /
