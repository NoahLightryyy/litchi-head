...
department: 数据管道部
codebase: src/data/
last_updated: 2026-07-30 (KR-1B-2C-2A 连续状态账本完成)
---
...
| 13 🔥 | K 线 KR-1B-2C-2B～3 官方采集与持久化 | 生命周期、批次/检查点、北交所、长窗与回放 |
| 14 🔥 | K 线 KR-2 统一复权 | RAW/公司行动/派生序列分层；点时因子 |
| 15 🔥 | K 线 KR-3 四层证据信封 | `FINAL_DAILY` 与今日 `PROVISIONAL` 严格分层 |
| 16 🔥 | 行业证据迁移到统一门禁 | K 线门禁完成后 |
...
4. KR-1B-2A/B/2C-1/2C-2A 已完成，不得重复状态契约、运行时门禁、CNINFO
   PDF 解析或连续状态归并；
5. 下一步为 KR-1B-2C-2B：实现沪深官方生命周期、CNINFO 完整查询批次与可持久化
   检查点生成；随后独立完成北交所适配；
