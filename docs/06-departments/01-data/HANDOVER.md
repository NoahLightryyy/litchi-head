...
| **FD-001f** 🥇 | **辩论注入财务数据** — `collect_data_node` 调用 `get_financials()`，存入 `market_data["financials"]` + 传入简报 | ✅ | FD-001e | ~1h |
...
DataCollector.get_financials(code)                        ← 缓存 TTL=1h ✅
     ↓
format_market_brief() → brief.sections["fundamentals"]    ← ✅ FD-001e 真实财务数据
     ↓
DebateState.market_data["financials"]                     ← ✅ FD-001f 消费中
DebateState.market_data["brief"]                           ← ✅ 分析师自动受益
