...
| 🔥 | **TD-069/KR-4 价格坐标门禁** — 下单/止损/止盈只使用 `LIVE_QUOTE/RAW`；复权技术位必须换回 RAW | 3B 审计运行时已完成但尚未接入交易链；等待 KR-2/4。交易只保存/核对 snapshot 引用，不直接读取 Parquet，不把回放证据当实时价格，不因部分 RAW 存在而放行 |
