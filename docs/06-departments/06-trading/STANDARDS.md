...
交易计划还必须携带 `evidence_envelope_id` 和 `price_as_of`。预期下单价、止损价和
止盈价只能基于 `LIVE_QUOTE/RAW` 的真实价格坐标；前复权/后复权只能提供技术距离，
使用时必须换算回 RAW 价格。证据不完整、RAW 冲突、复权冲突、行情过期或风控拒绝
时不得生成可执行订单。
...
- [ ] 交易记录持久化？
- [ ] 风控不通过有日志？
- [ ] 下单/止损/止盈价格是否处于 RAW 真实价格坐标？
- [ ] 交易记录能否追溯到证据信封和 `price_as_of`？
