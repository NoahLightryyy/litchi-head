...
| K 线 RAW/复权血缘（data→storage→backtest/trading） | 🟡 | KR-3A 四层契约/晋升核心完成；下一步 KR-3B 运行时组装，事件和累计快照仍不得单独进入下游 |
...
| 🥇 | **ADR-013 / TD-069 K 线完整性** — KR-3A 完成；按 [KR-3B～KR-6](../../02-requirements/KLINE_EVIDENCE_IMPLEMENTATION_PLAN.md) 推进运行时组装与下游验收 | 数据、辩论、Agent、记忆、风控、交易、回测、后端、前端、基础设施、质量保障 |
