...
| 多源证据（data→debate/backend） | 🟡 | 契约完成；CNINFO 直连三态门禁通过，统一汇总服务待接入 |
...
| 🥇 | **ADR-013 多源证据完整性** — CNINFO 直连三态门禁完成；统一汇总信封、新闻双源、DataEvidenceService 和正式失败关闭待推进 | 数据管道部 + 辩论引擎部 + 后端 API 部 |
