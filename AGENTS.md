...
# 遇到复杂任务时，临时改为 deepseek-v4-pro，重启 Codex
```
...
你下班/说"写进文档"
  → 我自动：
      ├── 完整收尾（CLOSING.md §2 完整收尾 checklist）  ← 全自动
      ├── 更新日志 + 债务 + 看板                       ← 全自动
      └── 提交                                        ← 全自动
```

> **强制闸门**：每个原子功能交付后、问用户"下个方向"之前，自动执行。
> 详见 [CLOSING.md §1 强制闸门](docs/01-guides/workflow/CLOSING.md#1-强制闸门每次功能交付后执行)。
>
> **红线**：不得在编码完成后、闸门未过的状态下问用户"下一步"。

**不需要停的事**（强制闸门中自动处理，详见 [CLOSING.md §1](docs/01-guides/workflow/CLOSING.md#1-强制闸门每次功能交付后执行)）：
- 测试没过 → 自己修
- Lint 没过 → hook 自动修（仅限本次改的文件）
- **CI 健康自检** → 闸门中自动跑 `ruff check .` + `pyright src/`，小问题顺手修，修不了的登记新债务，**不在合并时留红**
- 小重构 → 顺手做
- 技术细节选择（A 库还是 B 库）
